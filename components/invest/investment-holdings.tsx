  ...
  return <>
    ...
    {summary.holdings.length ? <div className="investment-list">{summary.holdings.map((holding) => <article className="stack-card investment-holding" key={holding.id}>
      <form onSubmit={(event) => onSave(event, holding.id)}>
        <div className="investment-holding-head"><strong>{holding.name}</strong><button type="button" className="workbook-icon-button" aria-label={`Remove ${holding.name}`} disabled={pending} onClick={() => run(() => deleteInvestmentHoldingAction(holding.id))}><Trash2 size={15} /></button></div>
